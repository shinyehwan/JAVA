...
class Main4 {
    public static void main(String[] args) {
         ...
         * 동물 : 숨쉬다
         * 원숭이 : 숨쉬다, 원숭이묘기를하다.
         * // 'a원숭이2' 변수에는 버튼 몇개짜리 리모콘? 2개
         * // 'a동물' 변수에는 버튼 몇개짜리 리모콘? 1개
         * 원숭이 a원숭이2 = a동물;
         *
         * 버튼 1개를 추가하면 되는거다.
         * 리모콘에서 버튼을 추가하는 것은
         ...
         * - 안될 수 도 있다.
         *
         * 자바에서는 약간이라도 위험에 대한 가능성이 존재한다면 넘어가 주지않는다.
         * 누가? 컴파일러가!
         *
         * -> 되려면 형변환을 해야한다.
         *
         * 추상클래스는 객체 생성이 안된다. 동물 a동물 = new 동물(); (X)
         *
         *
         * 사람 a사람 = new 사람();
         * 원숭이 a원숭이 = new 원숭이();
         *
//         동물 a동물 = a원숭이; // 동물이기만 하면 된다 -> 버튼 2개짜리를 하나 빼면 된다.
         * if 된다고 하면
         * 동물 a동물 = a사람; // 동물이기만 하면 된다 -> 버튼 2개짜리를 하나 빼면 된다.
         * a원숭이 = a동물; // 결국 리모콘 버튼을 늘려야한다.
         * a원숭이.숨쉬다(); // 사람이
         * a원숭이.묘기(); // 사람이
         *
         * 결론:
         ...
         * 원숭이 a원숭이 = new 원숭이();
         *
         * 동물 a동물 = a원숭이; // 자바에서는 안전한 것에 대해서는 자동형 변환
         * 원숭이 a원숭이2 = (원숭이)a동물; // 흐름상 되는 걸 왜 똑똑한 자바가 모를까? 컴파일러가 모르는 것. 확실한 개발자의 의지표명 요구.
         * // 자바에서는 안전하지 않을 수 있는 것에 대해서는 개발자의 사인이 필요하다. => 수동형 변환
         *
         * 사람 a사람 = new 사람();
         * 원숭이 a원숭이 = new 원숭이();
         *
         ...
         * 원숭이 a원숭이2 = (원숭이)a동물;
         *
         * 컴파일 타임(error)이 아닌 런타임 오류가 발생함(exception)
         *
         */
        ...
        // - 상황 : 동물리모콘은 원숭이 객체를 가리키고 있다.
        // - 상황 : 그렇지만 자바는 동물리모콘에 연결된 것이 동물객체라는 것은 알지만 그게 사람객체인지 동물객체인지는 알 수 없다.
        // - 상황 : `a원숭이2` 변수는 오직 원숭이 객체랑만 연결될 수 있다.
        // - 상황 : 동물리모콘이 원숭이 리모콘으로 변해야 하는 상황이다.
        // - 상황 : 기존의 리모콘에 `원숭이묘기를하다` 버튼을 추가로 넣어야 하는 상황이다.
